...
def find_row_bounds(text: str, rows_start: int, rows_end: int):
    ...
    if row_start < 0:
        raise SystemExit("Found Montrose County marker but could not locate row start inside rows array")

    endings = []
    for token in ("},\n", "}\n,", "}\n];"):
        pos = text.find(token, marker_pos, rows_end + len(token))
        if marker_pos <= pos < rows_end:
            endings.append(pos)
    if not endings:
        raise SystemExit("Found Montrose County marker but could not locate row end inside rows array")
    row_end = min(endings) + 1
    if not (rows_start <= row_start < row_end <= rows_end):
        raise SystemExit("Montrose County row bounds escaped rows array")
