...
def extract_row(text: str) -> str:
    ...
    if pos < 0:
        raise SystemExit("Lincoln County Nebraska market row is missing")
    ...
    if start < rows_start or not endings:
        raise SystemExit("Could not safely isolate Lincoln County row")
...
def main():
    ...
    if missing:
        raise SystemExit("Lincoln County row missing required safety/source text: " + ", ".join(missing))
    ...
    if bad:
        raise SystemExit("Lincoln County row contains unsupported claim(s): " + ", ".join(bad))
    if "immediate ownership" in lower and "not immediate ownership" not in lower:
        raise SystemExit("Lincoln County row must preserve lien/certificate versus ownership distinction")
    if "no for the published 2026 procedure" not in lower:
        raise SystemExit("Lincoln County row must preserve the official in-person 2026 sale method")

    print("Lincoln County Nebraska market validation passed")
