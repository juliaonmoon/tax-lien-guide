...
def _replace_existing_row(text: str, start: int, end: int) -> tuple[str, bool]:
    ...
    if row_start < start:
        raise SystemExit("Found Maricopa marker but could not locate row start inside rows array")

    next_row = text.find("\n{state:'", marker_pos, end)
    if next_row >= 0:
        row_end = next_row
    ...
    if row_end > end or row_start < start:
        raise SystemExit("Refusing Maricopa row repair outside rows array")
...
def main():
    ...
    text, replaced = _replace_existing_row(text, start, end)
    if replaced:
