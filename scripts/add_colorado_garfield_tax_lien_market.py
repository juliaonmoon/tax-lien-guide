...
def find_row_bounds(text: str, rows_start: int, rows_end: int):
    ...
    if row_start < rows_start:
        raise SystemExit("Found Garfield County marker but could not locate row start inside rows array")
    endings = []
    for token in ("},\n", "}\n,", "}\n];"):
        ...
        if pos >= 0:
            endings.append(pos)
    if not endings:
        raise SystemExit("Found Garfield County marker but could not locate row end inside rows array")
