...
def find_row_bounds(text: str, rows_start: int, rows_end: int):
    ...
    if row_start < rows_start:
        raise SystemExit("Found Alamosa County marker but could not locate row start inside rows array")

    candidates = []
    for terminator in ("},\n", "}\n", "},\r\n", "}\r\n"):
        pos = text.find(terminator, marker_pos, rows_end)
        if pos >= 0:
            candidates.append((pos, terminator))
    if not candidates:
        raise SystemExit("Found Alamosa County marker but could not locate row end inside rows array")

    row_end, terminator = min(candidates, key=lambda item: item[0])
    row_end += 1
    if row_start < rows_start or row_end > rows_end:
        raise SystemExit("Refusing Alamosa County repair outside rows array")
