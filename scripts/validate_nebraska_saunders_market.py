...
REQUIRED = [
    ...
    "saunderscounty.ne.gov/treasurer-office/public-tax-sale-information/",
]
...
def extract_row(text: str) -> str:
    ...
    if pos < 0:
        raise SystemExit("Saunders County Nebraska market row is missing")
    ...
    if start < rows_start or not endings:
        raise SystemExit("Could not safely isolate Saunders County row")
...
def main():
    ...
    if missing:
        raise SystemExit("Saunders County row missing required safety/source text: " + ", ".join(missing))
    ...
    if bad:
        raise SystemExit("Saunders County row contains unsupported claim(s): " + ", ".join(bad))
    if "private tax sale" in lower and "not a claim that any particular parcel or certificate is currently available" not in lower:
        raise SystemExit("Saunders private-sale text must not imply current inventory")
    if "not the property" not in lower:
        raise SystemExit("Saunders row must preserve tax-certificate versus property-ownership distinction")
    if "no for the published procedure" not in lower:
        raise SystemExit("Saunders row must preserve the official in-person sale method")

    print("Saunders County Nebraska market validation passed")
