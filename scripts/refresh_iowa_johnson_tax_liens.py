...
names are intentionally not collected. The parser stops before the separate
mobile-home section so every emitted record is a real-estate parcel item.
"""
...
def parse_real_estate_rows(text: str, verified: str) -> list[dict]:
    ...
    while i < len(lines):
        ...
        item_index = item_number = None
        first_description = ""
        for j in range(i + 1, min(i + 5, len(lines))):
            match = ITEM_START.match(lines[j])
            if match:
                item_index = j
        ...
        for k in range(item_index, min(item_index + 32, len(lines))):
            candidate = lines[k]
            marker_seen_in_block = marker_seen_in_block or "PUBLIC BIDDER TAX SALE" in candidate
            if k > item_index:
            ...
            if amounts:
                ...
                description_parts[-1] = MONEY.sub("", description_parts[-1]).replace("PUBLIC BIDDER TAX SALE", "").strip()
                end_index = k
                break
            end_index = k
        ...
        redemption = (
            ...
            if row_public_bidder else
            "90-day notice of right of redemption may be issued after 1 year 9 months from sale"
        )
        rows.append({
            "record_id": f"IA-Johnson-2026-{item_number}", "profile_id": PROFILE_ID,
            "state": "IA", "state_name": "Iowa", "county": "Johnson",
            "parcel_id": parcel, "sale_item_number": str(item_number), "legal_description": legal,
            "auction_date": "2026-06-15", "sale_date": "2026-06-15", "auction_time": "08:00 CT",
            ...
            "auction_url": SOURCE_PAGE, "official_source_url": SOURCE_PDF, "direct_listing_url": SOURCE_PAGE,
            "minimum_bid": None, "opening_bid": None, "delinquent_tax_amount": published_total,
            ...
            "last_verified": verified, "source_mode": "official_2026_publication_snapshot",
            "data_completeness": {"published_fields": 8, "tracked_fields": 19, "percent": 42},
            "research_priority": {
                "score": 42, "label": "Low research priority",
...
def update_details(rows: list[dict]) -> None:
    ...
    profiles[PROFILE_ID] = {
        ...
        "county_information_url": "https://www.johnsoncountyiowa.gov/treasurer/tax-sale-information", "procedures_url": PROCEDURES,
    }
...
def main() -> None:
    try:
        ...
        if not prior:
            raise
        print(f"Johnson County IA: live official publication unavailable/unparseable; preserved {len(prior)} previously verified rows. Reason: {exc}")
        return
    update_details(rows)
