...
def find_row_bounds(text: str):
    ...
    if row_start < rows_start:
        raise SystemExit("Found Larimer County marker but could not locate row start inside rows array")

    # Rows in index.html use more than one separator style: `},\n{...}` and
    # `}\n,\n{...}`. Looking for `}\n` first is unsafe because a row that
    # actually ends `},\n` does not contain that byte sequence; the search can
    # then jump forward to a later row and replacement deletes every row in
    # between. Choose the nearest valid row terminator, but never search beyond
    # the market rows array.
    endings = []
    for token in ("},\n", "}\n,", "}\n];"):
        ...
        if pos >= 0:
            endings.append(pos)
    if not endings:
        raise SystemExit("Found Larimer County marker but could not locate row end inside rows array")
    row_end = min(endings)
