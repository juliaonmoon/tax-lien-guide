...
def find_row_bounds(text: str):
    ...
    if row_start < 0:
        raise SystemExit("Found Chaffee County marker but could not locate row start within rows array")
    ...
    for token in ("},\n", "}\n,", "}\n];"):
        ...
        if pos >= 0:
            endings.append(pos)
    if not endings:
        raise SystemExit("Found Chaffee County marker but could not locate row end within rows array")
