...
def find_row_bounds(text: str, rows_start: int, rows_end: int):
    ...
    if row_start < rows_start:
        raise SystemExit("Found Rio Blanco County marker but could not locate row start inside rows array")
    ...
    for token in ("},\n", "}\n,", "}\n];"):
        ...
        if pos >= 0 and pos <= rows_end:
            endings.append(pos)
    if not endings:
        raise SystemExit("Found Rio Blanco County marker but could not locate row end inside rows array")
    row_end = min(endings) + 1
    if row_end > rows_end:
        raise SystemExit("Rio Blanco County row repair would escape rows array")
