...
def find_row_bounds(text: str, rows_start: int, rows_end: int):
    ...
    if row_start < rows_start:
        raise SystemExit("Found Arapahoe County marker but could not locate bounded row start")
    ...
    if not candidates:
        raise SystemExit("Found Arapahoe County marker but could not locate bounded row end")
    row_end = min(candidates) + 1
    if row_end > rows_end:
        raise SystemExit("Refusing Arapahoe County repair outside rows array")
