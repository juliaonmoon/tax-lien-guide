...
def main():
    ...
    if start >= 0:
        ...
        if comma >= 0 and (end < 0 or comma < end):
            ...
            raise SystemExit("Could not find end of existing Marshall County row")
        existing = text[start:end]
        if existing == ROW:
            print("Iowa Marshall County canonical row already present")
            return
        INDEX.write_text(text[:start] + ROW + text[end:], encoding="utf-8")
        print("Restored canonical Iowa Marshall County tax-lien market row")
        return
