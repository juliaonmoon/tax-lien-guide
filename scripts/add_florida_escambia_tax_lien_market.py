...
def find_row_bounds(text: str):
    ...
    if row_start < 0:
        raise SystemExit("Found Escambia County marker but could not locate row start")
    ...
    for token in ("},\n", "}\n,", "}\n];"):
        ...
        if pos >= 0:
            endings.append(pos)
    if not endings:
        raise SystemExit("Found Escambia County marker but could not locate row end")
    row_end = min(endings)
...
def main():
    ...
    if bounds:
        ...
        if existing == ROW:
            print("Florida Escambia County row already canonical")
            return
        INDEX.write_text(text[:start] + ROW + text[end:], encoding="utf-8")
        print("Restored canonical Florida Escambia County tax-lien market row")
        return
