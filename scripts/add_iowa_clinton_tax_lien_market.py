...
def main():
    ...
    if start >= 0:
        ...
        # marker presence alone is not sufficient for an idempotent repair.
        end = text.find("}\n", start)
        ...
        if comma >= 0 and (end < 0 or comma < end):
            ...
            raise SystemExit("Could not find end of existing Clinton County row")
        existing = text[start:end]
        if existing == ROW:
            print("Iowa Clinton County canonical row already present")
            return
        INDEX.write_text(text[:start] + ROW + text[end:], encoding="utf-8")
        print("Restored canonical Iowa Clinton County tax-lien market row")
        return
