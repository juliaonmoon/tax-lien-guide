...
REQUIRED = (
    ...
    "https://thurstoncountyne.gov/treasurer-office/public-tax-sale-information/",
)
FORBIDDEN = (
    "guaranteed return",
    "guaranteed deed",
...
def extract_row(text: str) -> str:
    ...
    if marker < 0:
        raise SystemExit("Thurston Nebraska market row is missing")
    ...
    if row_start < start or not endings:
        raise SystemExit("Thurston Nebraska row boundaries are invalid")
...
def validate_calendar_event():
    ...
    if len(matches) != 1:
        raise SystemExit(f"Expected exactly one Thurston County calendar event, found {len(matches)}")
    event = matches[0]
    required_pairs = {
    ...
    if wrong:
        raise SystemExit("Thurston County calendar event has incorrect required fields: " + ", ".join(wrong))
    ...
    if missing:
        raise SystemExit("Thurston County calendar event missing safety/source text: " + ", ".join(missing))
    bad = [phrase for phrase in ("currently available parcels", "guaranteed return", "guaranteed deed") if phrase in text]
    if bad:
        raise SystemExit("Thurston County calendar event contains unsupported claim(s): " + ", ".join(bad))
...
def main():
    ...
    if missing:
        raise SystemExit("Thurston Nebraska row missing required safety/source facts: " + ", ".join(missing))
    bad = [value for value in FORBIDDEN if value.lower() in lowered]
    if bad:
        raise SystemExit("Thurston Nebraska row contains unsafe/unverified claims: " + ", ".join(bad))
    if "no for the published public-sale procedure" not in lowered or "in person" not in lowered:
        raise SystemExit("Thurston row must preserve the official in-person sale method")
    if "no particular parcel or certificate is asserted as currently available" not in lowered:
        raise SystemExit("Thurston row must not imply current private-sale inventory")
    if "not immediate property ownership" not in lowered or "buying delinquent taxes, not the property" not in lowered:
        raise SystemExit("Thurston row must explicitly preserve lien-versus-ownership separation")
    validate_calendar_event()
    print("Thurston Nebraska tax-lien market and calendar validation passed")
