...
def _row_bounds(text: str, start: int) -> tuple[int, int]:
...
def main():
    ...
    row_start = text.find("{state:'" + MARKER + "'")
    if row_start >= 0:
        start, end = _row_bounds(text, row_start)
        existing = text[start:end]
        if existing == ROW:
            print("Anne Arundel County Maryland canonical row already present")
            return
        INDEX.write_text(text[:start] + ROW + text[end:], encoding="utf-8")
        print("Restored canonical Anne Arundel County Maryland tax-lien market row")
        return
