...
def main():
    ...
    if start < 0:
        raise SystemExit("Kent County Maryland market row is missing")
    end = text.find("}\n", start)
    if end < 0:
        end = text.find("},", start)
    ...
    for expected in required:
        if expected not in row:
            raise SystemExit(f"Kent County Maryland safety/rule text missing: {expected}")

    for prohibited in ["owner_name:", "taxpayer_name:", "opening_bid:"]:
        if prohibited in row:
            raise SystemExit(f"Kent County Maryland row contains prohibited property field: {prohibited}")

    print("Kent County Maryland market safety validation passed")
