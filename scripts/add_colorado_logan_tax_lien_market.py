...
def find_row_bounds(text: str):
    ...
    if row_start < 0:
        raise SystemExit("Found Logan County marker but could not locate row start")

    # index.html has used multiple row separators over time. Evaluate every
    # supported terminator and choose the nearest so a repair cannot consume
    # adjacent market rows.
    endings = []
    for token in ("},\n", "}\n,", "}\n];"):
        ...
        if pos >= 0:
            endings.append(pos)
    if not endings:
        raise SystemExit("Found Logan County marker but could not locate row end")
    row_end = min(endings)
...
def main():
    ...
    if bounds:
        ...
        if existing == ROW:
            print("Colorado Logan County row already canonical")
            return
        INDEX.write_text(text[:start] + ROW + text[end:], encoding="utf-8")
        print("Restored canonical Colorado Logan County tax-lien market row")
        return
