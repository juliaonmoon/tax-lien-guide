...
def main():
    ...
    if all(x in html for x in wanted):
        print("All verified tax-lien market rows already present")
        return

    rows = []
    for row in NEW_ROWS.split("\n"):
        labels = [x for x in wanted if x in row]
        if labels and labels[0] in html:
            continue
        rows.append(row)
    if not rows:
